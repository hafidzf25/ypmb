            ...
            <div class="container-fluid" style="border: 2px solid #000000; border-radius: 10px;">
                <div class="row" style="background-color: #38B6FF;">
                    <div class="col pt-2 pb-2 d-flex align-items-center" style="border-bottom: 2px solid #000000;">
                        <span style="font-size: larger; font-weight:900">Edit {{ $data->nama_pelatihan }}</span>
                    </div>
                </div>
                <div class="row">
                    <div class="col pt-4 pb-4" style="background-color: white;">
                        ...
                        <a href="{{ url('/detailpelatihan/' . $data->id_pelatihan) }}" class="btn btn-success">Kembali ke halaman pelatihan</a>
                        @else
                        <div class="row mb-3">
                        ...
                        <hr>
                        <form action="{{ route('pembayaran.edit') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row mb-4">
                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col">
                                    <input type="file" class="form-control" id="inputFoto" name="bukti_pembayaran" multiple>
                                </div>
                            </div>
                            <div class="row mb-3">
                                <div class="col d-flex justify-content-center">
                                    <button type="submit" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop" style="width: 40%; border-radius: 10px; background-color:#38B6FF">Update Pembayaran</button>
                                </div>
                            </div>
                            <input type="hidden" name="id_user" value="{{ auth()->user()->id }}">
                            <input type="hidden" name="id_pelatihan" value="{{ $data->id_pelatihan }}">
                            <input type="hidden" name="id_partisipan_pelatihan" value="{{ $datapartisipan->id_ppp }}">
                        </form>
                        @endif
                    </div>
                </div>
